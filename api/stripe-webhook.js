...
<tr><td align="center" style="padding-top:16px;"><p style="font-size:11px;color:#B8B8AA;margin:0;line-height:1.6;">You're receiving this because you subscribed at <a href="https://verrixai.com" style="color:#B8963E;text-decoration:none;">verrixai.com</a>.<br/>Questions? Reply to this email or contact <a href="mailto:admin@verrixai.com" style="color:#B8963E;text-decoration:none;">admin@verrixai.com</a>.</p></td></tr>
      ...
      case 'customer.subscription.deleted': {
        ...
        // Send cancellation-finalised email to user (best-effort — failure here doesn't block the downgrade or admin notify)
        try {
          if (RESEND_API_KEY) {
            const userRes = await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${userId}`, {
              headers: { 'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`, 'apikey': SUPABASE_SERVICE_KEY }
            ...
            const userData = await userRes.json();
            if (userData.email) {
              const oldPlan = sub.metadata?.plan;
              const oldPlanLabel = oldPlan === 'starter' ? 'Starter' : oldPlan === 'pro' ? 'Pro' : oldPlan === 'pro2' ? 'Pro 2' : 'paid';
              const html = `<!DOCTYPE html>
<html><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/></head>
<body style="margin:0;padding:0;background:#F7F6F2;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F7F6F2;padding:40px 20px;">
...
<p style="font-family:Georgia,serif;font-size:24px;font-weight:500;color:#1A1A18;text-align:center;margin:0 0 10px;line-height:1.3;">Your subscription has ended</p>
<p style="font-size:15px;color:#6B6B62;text-align:center;margin:0 0 28px;line-height:1.65;font-weight:300;">As scheduled, your VerrixAI ${oldPlanLabel} subscription has now ended. You won't be charged again, and your account has been moved to the Free plan.</p>
<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;"><tr><td style="background:#FAF8F2;border:1px solid #E8E2D2;border-radius:10px;padding:18px 20px;">
<p style="font-size:12px;font-weight:600;color:#1A1A18;margin:0 0 10px;letter-spacing:0.04em;text-transform:uppercase;">Your Free plan</p>
<p style="font-size:13px;color:#6B6B62;margin:0 0 6px;line-height:1.6;">→ 3 scans available on your account</p>
<p style="font-size:13px;color:#6B6B62;margin:0 0 6px;line-height:1.6;">→ All core analysis features</p>
<p style="font-size:13px;color:#6B6B62;margin:0;line-height:1.6;">→ No card on file, nothing to manage</p>
</td></tr></table>
<p style="font-size:14px;color:#6B6B62;margin:0 0 28px;line-height:1.7;">If you'd like to subscribe again at any point, you can pick a plan from <a href="https://verrixai.com/pricing" style="color:#2E5C42;">verrixai.com/pricing</a>. Your account, history and settings are still here when you need them.</p>
<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding-bottom:8px;"><a href="https://verrixai.com" style="display:inline-block;background:#1A3A2A;color:#FFFFFF;font-size:14px;font-weight:500;text-decoration:none;padding:13px 32px;border-radius:12px;letter-spacing:0.02em;">Continue on Free</a></td></tr></table>
</td></tr>
<tr><td align="center" style="padding-top:24px;"><p style="font-size:11px;color:#B8B8AA;margin:0;line-height:1.6;">You're receiving this because your subscription period ended at <a href="https://verrixai.com" style="color:#B8963E;text-decoration:none;">verrixai.com</a>.<br/>Questions? Reply to this email or contact <a href="mailto:admin@verrixai.com" style="color:#B8963E;text-decoration:none;">admin@verrixai.com</a>.</p></td></tr>
</table>
</td></tr>
</table>
</body></html>`;
              ...
              await fetch('https://api.resend.com/emails', {
                ...
                body: JSON.stringify({
                  from: 'VerrixAI <admin@verrixai.com>',
                  to: userData.email,
                  subject: 'Your VerrixAI subscription has ended',
                  html,
                  text
                })
              });
          ...
          // Non-fatal — downgrade already applied to DB
          console.error('Cancel-finalised email failed (downgrade still applied):', emailErr);
        }
